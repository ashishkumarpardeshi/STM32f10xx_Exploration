...
int main(void)
{
	...
	GPIO_WritePin(GPIOB, GPIO_PIN_NO_13, GPIO_PIN_SET);		// Activate Pull-up on PB13

	redled.pGPIOx = GPIOA;
	redled.GPIO_PinConfig.GPIO_PinMode = GPIO_MODE_OUTPUT_2MHZ;
	redled.GPIO_PinConfig.GPIO_PinNumber = GPIO_PIN_NO_8;
	...
	while(1)
	{
		if(GPIO_ReadPin(GPIOB, GPIO_PIN_NO_13) == 0)
		{
			delay_ms(20);		// delay for debouncing
			while(GPIO_ReadPin(GPIOB, GPIO_PIN_NO_13) == 0);
			//{
